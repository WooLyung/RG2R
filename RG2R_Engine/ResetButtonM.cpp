...
ResetButtonM::ResetButtonM(bool isAnim, MapCreateScene* scene)
{
	this->isAnim = isAnim;
	this->scene = scene;
}

ResetButtonM::~ResetButtonM()
{
}

void ResetButtonM::OnStart()
{
	spriteRenderer = AttachComponent<SpriteRenderer>()
		->SetTexture("Resources/Sprites/UIs/Menus/resetButton.png")
	...
	appearAnim->PushCommand([=]() {
		...
		transform->SetPos(GetScene()->GetMainCamera()->GetCameraDefaultSize().width * 0.5f - 1.2f,
...
void ResetButtonM::OnUpdate()
{
	Input();
}

void ResetButtonM::Input()
{
...
Transform* ResetButtonM::GetTransform()
{
	return transform;
}

SpriteRenderer* ResetButtonM::GetSpriteRenderer()
{
	return spriteRenderer;
}

void ResetButtonM::Disappear()
{
	disappearAnim->Start();
	animTime = 0;
}